...
class Signup extends CI_Controller {
  ...
  /*index*/
    ...
    if ($this->input->post('step') == 1) {
        ...
        if ($checkemail) {
          return $this->output->set_output(json_encode(array('id' => 'email_result', 'status' => 'error', 'msg' => '<label class="text-danger"><span class="glyphicon glyphicon-remove"></span> Email Already register</label>')));
          exit;
        }
        $checkphone = $this->Main_model->is_phone_available($this->input->post('phone'));
        if ($checkphone) {
          return $this->output->set_output(json_encode(array('id' => 'phone_result', 'status' => 'error', 'msg' => '<label class="text-danger"><span class="glyphicon glyphicon-remove"></span> Phone Already register</label>')));
          exit;
        }
